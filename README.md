...
- [FeedInMngmt_Base_Models.ipynb](FeedInMngmt_Base_Models.ipynb): Creation and elaboration of train-validation-test split as well as design of two naive baseline models: the Shift model (e.g., the next timestep equals the last time step) and a Moving Average Model (e.g., next timestep equals the moving average over the last 2 hours). In addition, functions to evaluate the test results are described in this document.  
